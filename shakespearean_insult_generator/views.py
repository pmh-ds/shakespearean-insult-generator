...
@app.route('/')
def insult():
    return "Thou {insult}".format(insult=generate_insult(insults_start, insults_middle,  insults_end))
    
@app.route('/<name>')
def insult_name(name):
    return "{name}, thou {insult}".format(name=name, insult=generate_insult(insults_start, insults_middle,  insults_end))